    ...
    while(1){
        ...
        if(!strcmp(aux_dir->nome,para->valor)){                             //testa o nome
            if(aux_dir->filho==NULL)                                        //testa se esta vazio
                if(aux_dir->criador==usuario_atual||usuario_atual==root){   //testa se o usuario tem permição
                    if(aux_dir->ishome==0){                                 //testa se é home de um usuario
                        if(diretorio_atual->filho==aux_dir)                 //testa se é o filho ou um irmao
